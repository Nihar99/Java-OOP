...
public class CoronaCountryMap extends PApplet {
	...
	public void setup()
	{
		...
		{
			countryMarkers.add(createMarker(country));
		}
	...
	{  
		SimplePointMarker marker = new SimplePointMarker(((PointFeature)feature).getLocation(),feature.getProperties());
	    int red = color(255, 0, 0);
	    int orange = color(255,175,0);
	    int green = color(0,255,0);
	    if(Integer.parseInt(feature.getStringProperty("confirmed").toString()) <= GREEN_ZONE)
			marker.setColor(green);
		else if(Integer.parseInt(feature.getStringProperty("confirmed").toString()) > ORANGE_ZONE)
			marker.setColor(red);
		else
